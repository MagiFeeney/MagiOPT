...
def plot_surface(func, sequence, info=None):
    sequence, smax, smin = transform(sequence)
...
def transform(sequence):
    neq_seq = torch.vstack(sequence)
    smax, _ = torch.max(neq_seq, axis=0)
    smin, _ = torch.min(neq_seq, axis=0)

    return neq_seq, smax, smin
