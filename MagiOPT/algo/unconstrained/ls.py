...
class LeastSquares:
    ...
    def plot(self, info=None):
        ...
        if self.sequence[0].shape[0] <= 2:
            if info == None:
                plot_surface(self.Surrogate, self.sequence)
            else:
                plot_surface(self.Surrogate, self.sequence, info)
        if self.sequence[0].shape[0] == 2:
            if info == None:
                plot_contour(self.Surrogate, self.sequence)
            else:
                plot_contour(self.Surrogate, self.sequence, info)
